...
# Lazy import for T2S (only needed for text-to-speech, not audio encoding/decoding)
...
# Lazy import for g2p functions (only needed for text-to-speech, not audio encoding/decoding)
_g2p_funcs = None

def _get_g2p_funcs():
    global _g2p_funcs
    if _g2p_funcs is None:
        from models.tts.maskgct.g2p.g2p_generation import g2p, chn_eng_g2p
        _g2p_funcs = {'g2p': g2p, 'chn_eng_g2p': chn_eng_g2p}
    return _g2p_funcs

def g2p_(text, language):
    """Lazy-loaded g2p function - only imports g2p modules when actually called"""
    funcs = _get_g2p_funcs()
    if language in ["zh", "en"]:
        return funcs['chn_eng_g2p'](text)
    else:
        return funcs['g2p'](text, sentence=None, language=language)


def build_t2s_model(cfg, device):
    # Lazy import - only import when actually needed (for T2S functionality)
    try:
        from models.tts.maskgct.maskgct_t2s import MaskGCT_T2S
    except ImportError:
        raise ImportError(
            "maskgct_t2s module not found. T2S (text-to-speech) functionality requires "
            "the maskgct_t2s.py file which was not included in the minimal repository. "
            "This is only needed for text-to-speech, not for audio encoding/decoding."
        )
...
def build_semantic_model(device):
    ...
    # Find stats file relative to this file's location
    _file_dir = os.path.dirname(os.path.abspath(__file__))
    stats_path = os.path.join(_file_dir, "ckpt", "wav2vec2bert_stats.pt")
